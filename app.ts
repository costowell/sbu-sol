...
type Match = [Wrestler | Match, (Wrestler | Match)?];
...
function processMatches(matches: Match) : Wrestler {
  ...
  if (matches.length < 2) return wrestlerA;
  let wrestlerB = Array.isArray(matches[1]) ? processMatches(matches[1]) : matches[1] as Wrestler;
...
function runTournament(wrestlers: Wrestler[]) {
  ...
  let winner = processMatches(matches as Match);
